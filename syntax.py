...
def make_parser():
  ...
  def make_fancy_grammar(prec_order, ps):
    ...
    #
    # With this precedence-climbing setup, the exponential blowup seems confined to parentheses.
    # Specifically, strings
    #   (a1) .. (an)
    # have parse count exponential in n. This occurs when there are multiple
    ...
    # in the test cases below. This blowup should be removable: in all test
    # cases below, all parse trees are equal up to trimming (removal of nodes
    # for nonterminals whose names start with ?) so in some sense the conflict
    # shouldn't matter.
    #
