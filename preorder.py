...
class Preorder:
  ...
  representing the minimal collection of inequalities whose transitive closure
  recovers the ordering. For example, for the poset generated by the preorder
    a <= b
    b <= a
    a <= c <= d
    a <= d
  The skeleton is the DAG
    {a, b} <= {c} <= {d}
